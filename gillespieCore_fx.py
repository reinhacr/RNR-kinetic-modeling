...
def parseGillespie(infile):
    ...
    headerShouldBe = " ".join(["species_idx", "species_names", "counts", "counts_max", "reaction", "reaction_idx", "rate_constants", "species", "stoich", "n_reactants"])

    if not headerStr.startswith(headerShouldBe):
        raise ValueError("headers must be "
                         "species_idx, species_names, counts, counts_max, reaction, reaction_idx, rate_constants, species, stoich, n_reactants")

    speciesInfo = df[["species_idx", "species_names", "counts", "counts_max"]].dropna()
    rxnInfo = df.drop(["species_idx", "species_names", "counts", "counts_max"], axis=1).dropna()

    param = SimpleNamespace()
    param.speciesNames = speciesInfo.species_names.values

    param.counts = np.array(speciesInfo.counts.values).astype(np.int)
