...
def exponentialFitTrajectory(times, counts, minRSquared=0.95):
    ...
    # build rate matrix
        ...
        nSteps = 400
        # The below section was motified to collect other radical concentrations, transpose them, and then save to a csv
        ...
        countsY731 = np.array([(expm(-odeMatrix * t) @ param.initial_counts)[2] for t in times]) 
        countsY730 = np.array([(expm(-odeMatrix * t) @ param.initial_counts)[3] for t in times])
        countsC439 = np.array([(expm(-odeMatrix * t) @ param.initial_counts)[4] for t in times])
        #print(times)
        df_times = pd.DataFrame(times)
        df_counts = pd.DataFrame(counts)
        df_y731 = pd.DataFrame(countsY731)
        df_y730 = pd.DataFrame(countsY730)
        df_C439 = pd.DataFrame(countsC439)
        df_countsT = df_counts.T
        df_timesT = df_times.T
        df_countsY731T = df_countsY731
        df_countsY730T = df_countsY730
        df_countsC439T = df_countsC439
        print(df_countsT)
        print(countsY731T)
        print(countsY730T)
        print(countsC439T)
        radicals = pd.DataFrame([df_timesT,df_countsT,countsY731T,countsY730T,countsC439T], columns=['Times','Y356','Y731','Y730','C439'])
        radicals.to_csv('radicals_pop.csv')
